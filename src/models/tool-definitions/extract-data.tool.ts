...
export function getExtractDataToolDefinition(): FunctionTool {
  return {
    ...
    description: description(
      "Builds a structured query against the virtual table and renders the result as CHART, TABLE or TEXT.",
      "Use when the user is asking for data or insights, even if they don't explicitly ask for a chart or table.",
      "The virtual table fields (with fieldType) are provided in the conversation context.",
      "HARD PRECONDITION: any WHERE/HAVING filter targeting a field with fieldType='string' MUST use values returned by a prior request_field_values call in this conversation, OR use LIKE/STARTS_WITH (only allowed when the user denied request_field_values). Never guess string values.",
      "If the user's request implies a string filter but no request_field_values has been made yet for that field, do NOT call this tool — call request_field_values first.",
      "If any other required detail is missing or ambiguous (date range, metric, grouping), call ask_followup instead.",
    ),
    parameters: {
      type: "object",
      properties: {
